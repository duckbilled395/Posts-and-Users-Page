...
const PostsContainer: FC = React.memo(() => {
	...
	const selectionOfPostsOutput = (posts: Array<PostsType>, userId?: string | null | undefined) => {
		if (!userId) {
			return posts
				...
				.map(element => <Post title={element.title} text={element.body} postId={element.id}
									  key={element.id} />)
		} else {
			let filteredPostsByUserId = posts.filter(post => post.userId === +userId)
			return filteredPostsByUserId.map(element => <Post title={element.title} text={element.body}
															  postId={element.id} key={element.id} />)
		}
	}
	...
	if (match) {    // Choosing should it be default /posts/ or /posts/user:userId
		...
		return (
			<div>
				<Paginator itemsAmount={posts.length} pageSize={pageSize} setCurrentPage={setCurrentPage}
