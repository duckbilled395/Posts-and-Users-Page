...
const Paginator: FC<PropsType> = React.memo(({itemsAmount,setCurrentPage, currentPage, pageSize = 10}) => {
    ...
    let pageButtons = pages.map(page => {
        return currentPage === page
            ? <StyledActivePageButton key={page}
                                      onClick={() => setCurrentPage(page)}>{page}
             </StyledActivePageButton>
            : <span key={page} onClick={() => setCurrentPage(page)}>{page}</span>
    })

    return (
        <StyledPaginatorContainer>{pageButtons}</StyledPaginatorContainer>
    )
}, (prevProps, nextProps) => {
